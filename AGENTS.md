...
- **Read path is read-only; act path is gated.**
  ...
  - `install-harness` (including `--uninstall`) is a bootstrap verb: it applies
    by default (`--dry-run` is opt-in). `--uninstall` removes only acb-owned
    artifacts whose on-disk content exactly matches what acb would render today
    (hash check); marker-bearing shims with changed content are preserved (user
    edits are never destroyed). Its JSON uses the shared closed status
    vocabulary: a preserved ownership conflict is `failed` with explicit
    conflict metadata, never a new `conflict` status. MCP removals back up
    config first.
...
`codex` is part of the closed suite harness set. `install-harness codex` renders
 cred discovery skills into `$HOME/.agents/skills/<name>/SKILL.md` (Codex's own
 skill format in the user-scoped shared skills tree), backup-first and create-only
 — the `CodexAdapter` (Plan 008 WI-3.1). It stays **out of the stable `all`
 expansion** (Decision 2) until the live interop proof (Plan 007 WI-3.1) lands;
`CODEX_HOME`/`ACB_CODEX_HOME` select the config root and `ACB_HOME` selects the
shared skills home. For isolated discovery tests, launch Codex with `HOME` set
to that same `ACB_HOME`; Codex itself discovers `$HOME/.agents/skills` and does
not interpret ACB's override. The component-owned `plugins/acb` bundle carries
only generic, value-free broker guidance; manifest-specific credential skills
remain generated by the direct installer. Codex MCP writes (e2e provider)
remain an honest `unsupported` result. Hermes remains a component-private
explicit target and is not part of the suite's stable `all` expansion.
